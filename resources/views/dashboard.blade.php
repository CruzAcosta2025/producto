    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="gap-4">
            <div class="bg-gray-800 p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 my-6">
                ...
                <ul class="space-y-3">
                    <li class="text-lg font-medium text-white"><strong>Total de proyectos:</strong> {{ json_decode($totalProyectos) }}</li>
                </ul>
            </div>
        </div>
        ...
        <div class="gap-4">
            <div class="bg-gray-800 p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 my-6">
                ...
                <ul class="space-y-3">
                    <li class="text-lg font-medium text-white"><strong>Total de tareas:</strong> {{ json_decode($totalTareas) }}</li>
                </ul>
            </div>
        </div>
        ...
        <div class="gap-4">
            <div class="bg-gray-800 p-6 rounded-lg shadow-md my-6">
                ...
                <ul class="space-y-2 mt-4">
                    @foreach (json_decode($proyectosProximos) as $proyecto)
                    <li class="text-lg font-medium text-white">
                        {{ $proyecto->nombre }} - Vence el {{ \Carbon\Carbon::parse($proyecto->fecha_fin)->format('d-m-Y') }}
                    </li>
                    @endforeach
                </ul>
            </div>
        ...
        <!-- Recordatorios de proyectos atrasados -->
        <!-- <div class="gap-4">
            <div class="bg-red-800 p-6 rounded-lg shadow-md my-0">
                <h3 class="text-xl font-semibold text-white">Proyectos Atrasados</h3>
                <ul class="space-y-2 mt-4">
                    @foreach (json_decode($proyectosAtrasados) as $proyecto)
                    <li class="text-lg font-medium text-white">
                        {{ $proyecto->nombre }} - Venció el {{ \Carbon\Carbon::parse($proyecto->fecha_fin)->format('d-m-Y') }}
                    </li>
                    @endforeach
                </ul>
            </div>
        ...
        <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
            <div class="flex justify-center mb-6">
                <a href="{{ url('/jitsi') }}"
                    ...
                    <svg class="w-5 h-5 mr-2 text-gray-800" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a4 4 0 00-8 0v2M5 12h14M12 19v-7" />
                    </svg>
                    Crear Sala Meet
                </a>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var proyectos = @json(json_decode($proyectos)); // Los proyectos recibidos desde el controlador
        ...
        var tasksProgressChart = new Chart(ctx3, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: [tareasCompletadas, tareasPendientes], // Tareas completadas vs pendientes
